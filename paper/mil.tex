...
To show that this model generalizes well, we perform similar tests on the RisQ smoking dataset. In this case, we demonstrate that the performance increases as the number of labeled sessions increases. Figure \ref{fig:figure4} shows the average LOPO f1 score of the sbMIL classifier for various number of labeled sessions. We see nearly a 10\% increase in performance given a small number
