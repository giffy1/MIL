...
def get_param_grid_by_clf(clf_name, kernel='linear'):
	...
	class_weights = [{1 : float(i)/(i+j), -1 : float(j)/(i+j)} for (i,j) in farey(20)[1:]] #ignore first value where i=0
	class_weights = [{1 : float(j)/(i+j), -1 : float(i)/(i+j)} for (i,j) in farey(10)[1:]] #swap i and j, ignore first value
	...
	if clf_name in {'SIL', 'sMIL', 'sbMIL', 'RF', 'SVM', 'LinearSVC'}: # {'SVM', 'LinearSVC'}: # {'SIL', 'sMIL', 'sbMIL', 'RF', 'SVM', 'LinearSVC'}:
