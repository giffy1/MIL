...
def load_data(data_dir = './', frame_size = 144, step_size = 72, units = 'u', pickle_path = './data.pickle'):
	...
	@param data_dir : 	The directory where the dataset is located. The default is 
				the current working directory' which assumes this script 
	...
	if pickle_path != 'none':
		...
		try:
			...
			print("Failed to load data from " + str(pickle_path))
			pass
...
# --------------- Ground Truth - Get times for all activities --------------
	...
	dataset = {'data': {'X': X, 'Y': Y, 'sessions' : {'start' : activities_time, 'labels' : activities_eatingflag}}, \
		     'parameters' : {'frame_size': frame_size, 'step_size': step_size, 'units': units}, 'description' : "Lab-20 Eating Dataset"}
	...
	if pickle_path != 'none':
		try:
			...
			print("Failed to save data to " + str(pickle_path))
			pass
	return dataset
